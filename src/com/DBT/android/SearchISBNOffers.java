...
public class SearchISBNOffers extends AsyncTask<String, Void, String>{
	static JSONArray jObj = null;
	static JSONObject book_object=null;
	static StringBuilder builder;
	public static ArrayList<OfferBeam> offers_ISBN=null;
	...
	private static void Get_Offers_aroundme(String isbn) throws ClientProtocolException, IOException{
		HttpClient client = new DefaultHttpClient();
		String url ="http://dontburnthem.herokuapp.com/api/offers/search";
		HttpGet request = new HttpGet(url);
		HttpParams p = request.getParams();
		p.setParameter("isbn", isbn);
		request.setParams(p);
	...
	private static void Parse_Result(String readed) throws JSONException, ClientProtocolException, IOException{
		...
		offers_ISBN = new ArrayList<OfferBeam>(jObj.length());
		...
		System.out.println("Addo:" + ob.getBookbeam().getAuthors() + " " + ob.getBookbeam().getTitle());
		offers_ISBN.add(ob);
		}
    ...
    String ISBN;
	public SearchISBNOffers (Context ctx, AsyncTaskCompleteListener<String> cb, String ISBN){
		this.ISBN=ISBN;
		this.ctx = ctx;
		this.callback= cb;
	}
	...
	protected void onPostExecute(String result){
		callback.onTaskComplete("Completed");
		dialog.dismiss();
	}
